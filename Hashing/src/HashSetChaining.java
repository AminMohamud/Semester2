...
public class HashSetChaining {
	...
	public boolean remove(Object x) {
		...
		if (previous.data.equals(x)) {
			//first index becomes 2nd index,

			buckets[h] = previous.next;
			currentSize--;
		} else {
			while (!found && previous.next != null) {
				if (previous.next.data.equals(x)) {
					//Kopi af previous.next
					Node current = previous.next;
					buckets[h] = previous.next;
					//Current next peger på det næste index, derfor udføres den næste linje.
